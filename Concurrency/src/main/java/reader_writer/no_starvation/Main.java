...
public class Main {
    ...
    private void execute() throws IOException {
        ...
        final ReentrantLock serviceQueue = new ReentrantLock(true);
        ...
        for (int i = 0; i < WRITER_COUNT; i++) {
            final int id = i;
            executor.execute(() -> writeLoop(new Writer(file, resource, serviceQueue), id));
        }

        for (int i = 0; i < READER_COUNT; i++) {
            final int id = i;
            executor.execute(() -> readLoop(new Reader(file, resource, serviceQueue, readMutex), id));
        }
