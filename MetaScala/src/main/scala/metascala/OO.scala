...
object OO {
  ...
  class Method1[Obj, I, O](fn: (Obj, I) => O) extends Method1Base[Obj] {
    ...
    def apply(obj: Obj, arg: In1): Out = fn(obj, arg)
  }
  ...
  case class RichHCons[H, T <: HList](l: HCons[H, T]) {
    ...
    def |=[M <: MethodBase](m: M)(implicit fn: ReplaceByTypeFn[HCons[H, T], _0, M]): _root_.metascala.HLists.HCons[H, T] = _override(l, m)
  }
  ...
  case class MethodProduct[P <: Product](p: P) {
    ...
    def |=[M <: MethodBase](m: M)(implicit fn: Replacer[P, M]): P = replace(p, m)
  }

  implicit def productToMethodProduct[P <: Product](p: P): MethodProduct[P] = MethodProduct(p)
