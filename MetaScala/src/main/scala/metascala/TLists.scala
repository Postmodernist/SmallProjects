...
object TLists {
  ...
  implicit def containsFn[H, T <: TList]: ContainsFn[H, H :: T] = ContainsFn[H, H :: T]()

  implicit def containsFn2[H, H2, T <: TList](implicit fn: ContainsFn[H, T]): ContainsFn[H, H2 :: T] = ContainsFn[H, H2 :: T]()
  ...
  implicit def removeFirstFn[H, T <: TList]: RemoveFirstFn[H, H :: T, T] = RemoveFirstFn[H, H :: T, T]()

  implicit def removeFirstFn2[H, H2, T <: TList, RT <: TList](implicit fn: RemoveFirstFn[H, T, RT]): RemoveFirstFn[H, H2 :: T, H2 :: RT] = RemoveFirstFn[H, H2 :: T, H2 :: RT]()
  ...
  implicit def isSubSetFn[T <: TList]: IsSubSetFn[TNil, T] = IsSubSetFn[TNil, T]()

  implicit def isSubSetFn2[H, T <: TList, T2 <: TList](implicit fn1: ContainsFn[H, T2], fn2: IsSubSetFn[T, T2]): IsSubSetFn[H :: T, T2] = IsSubSetFn[H :: T, T2]()
